...
save_path = r'./model_test/all/4_paper_epoch1_Deep/'
...
def objective(trial,dataset_info_pretrain,dataset_info_finetune):
    ...
    try:
        ...
        if No_i < 1:
            ...
            # 用于比较看weight_decay_pretrain = 0.4 是不是有效
            depth = 8
        elif  No_i<3:
            depth = 12    
            ...
            depth = 8  
            weight_decay_pretrain = 0.3
            trial.set_user_attr('weight_decay_pretrain', "0.3two:v1.1")
        ...
        # 定义当前工作路径
        model_working_path = os.path.join(save_path, "{}_depth=[{}]/".format(No_i,depth))
        No_i=No_i+1
...
#***************************微调*************************************
        ...
        # 定义预训练迁移路径  
        args_fine_tune.finetune = r"G:\Star\model_test\all\4_paper_epoch1_Deep\0_depth=[8]\pretrain\SpecTE(Pa=[115]-Di=[160]-Ha=[16]-De=[8])\OP\weight_best.pkl"
...
def main():
    ...
    try:
        ...
        study.optimize(lambda trial: objective(trial, dataset_info_pretrain,dataset_info_finetune),n_trials=n_trials,callbacks=[save_study_at_every_step])
