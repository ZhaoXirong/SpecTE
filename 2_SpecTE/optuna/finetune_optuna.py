...
def objective(trial,dataset_info_pretrain,dataset_info_finetune):
    ...
    try:
        ...
        # 微调
        lr=0.0003                    # 2. 0.01        0.005         0.001     0.0005      0.02         
        batch = 256                         
        weight_decay=0.1             # 1. 0.2         0.3           0.1        0.05
        
        
        drop_rate=0.05                # 5  0.2         0.1          0.05       0.3
        attn_drop_rate=0.             # 3  0.05        0.1          0.01        0.                   
        patch_drop_rate= 0.           # 4  0.         0.01         0.1      
        drop_path_rate=0.13           # 6  0.05       0.1          0.
        pos_drop_rate = 0.13          # 7  0.1        0.01          0. 
        proj_drop_rate = 0.05         # 8  0.1        0.05         0.
        ...
        # 定义当前工作路径
        model_path = os.path.join(save_path, "{}_lr=[{}]-wd=[{}]/".format(No_i,lr,weight_decay))
        No_i=No_i+1
...
#         args_pretrain.Hyperparameters_MAE={'patch_size':patch_size, # 将输入图像分割成补丁的大小。
#                                  'embed_dim':dim, # 嵌入维度
...
#***************************微调*************************************
        ...
        # 定义工作路径
        args_fine_tune.path_log= os.path.join(model_path,"fine_tune/")
        ...
        best_loss,model_path_1 = train_finetune(args_fine_tune,dataset_info_finetune)
        model_path_list.append(model_path_1)
        trial.set_user_attr('none_loss', best_loss.item())


        # # two
        # args_fine_tune.parameter_group='two'
        # best_loss,model_path_2 = train_finetune(args_fine_tune,dataset_info_finetune)
        # model_path_list.append(model_path_2)
        # trial.set_user_attr('two_loss', best_loss.item())

        # # each
        # args_fine_tune.parameter_group='each'
        # best_loss,model_path_3 = train_finetune(args_fine_tune,dataset_info_finetune)
        # model_path_list.append(model_path_3)
...
# #************************************集成*****************************************
#         blending_args = parser.parse_args()
...
#         blending_args.path_save = os.path.join(model_path,"blending/")
